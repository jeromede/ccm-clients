/*
 * Copyright (c) 2017..2025 Jérôme Desquilbet <jeromede@fr.ibm.com>
 *
...
import xyz.ccm.client.utils.Login;
...
public class Write {
	...
	public static void main(String[] args) {
		...
		try {
			...
			complete = Boolean.valueOf(args[4]).booleanValue();
			...
			monitor.err(
					"example: https://test.example.com/ccm \\\"S0m3 Pr0j3ect UU_PP\\\" jazzadm P4ssw0rD true UU_PP.ser attachments_here members.txt ids.txt");
			monitor.err(
					"note: members_input_file has to be a UTF-8 xyz.ccm.client.write.text file with a line for each member; this line should read like:\n\tID_in_source ID_in_target");
			monitor.err(
	...
	static String matchingMembers(Map<String, String> map, String filename, String whenother) {
		try {
			...
			if (0 == whenother.length()) {
				e.printStackTrace();
				return "error reading UTF-8 xyz.ccm.client.write.text file " + filename;
			}
		}
		return null;
	}
	...
	private static String writeIdMatchFile(String filename, Project p) {
		...
		try {
			...
			e.printStackTrace();
			return "error creating UTF-8 xyz.ccm.client.write.text file " + filename;
		}
		return null;
	}
