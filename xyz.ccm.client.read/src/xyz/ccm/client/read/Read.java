...
public class Read {
	...
	public static void main(String[] args) {
		...
		try {
			...
			monitor.err(
					"example: https://test.example.com/ccm01 \"S0m3 Pr0j3ect UU_PP\" jazzadm P4ssw0rD true UU_PP.ser attachments_here");
			monitor.err(
		...
		monitor.out("arguments: url pa user password complete serialization_file attachment_dir:");
		monitor.out("CCM URL: " + url);
		monitor.out("Project Area: " + proj);
		monitor.out("User: " + user);
		monitor.out("Complete (with WI versions)? " + complete);
		monitor.out("Serialized .ser file: " + ser);
		monitor.out("Attachment Directory: " + dir);
